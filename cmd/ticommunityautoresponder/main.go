...
import (
	...
	"github.com/tidb-community-bots/ti-community-prow/internal/pkg/externalplugins/autoresponder"
...
func main() {
	...
	helpProvider := autoresponder.HelpProvider(epa.Config())
...
func (s *server) handleEvent(eventType, eventGUID string, payload []byte) error {
	...
	case "issue_comment":
		...
		go func() {
			if err := autoresponder.HandleIssueCommentEvent(s.gc, &ice, config, l); err != nil {
	...
	case "pull_request_review_comment":
		...
		go func() {
			if err := autoresponder.HandlePullReviewCommentEvent(s.gc, &pullReviewCommentEvent, config, l); err != nil {
				l.WithField("event-type", eventType).WithError(err).Info("Error handling event.")
			}
		}()
	default:
